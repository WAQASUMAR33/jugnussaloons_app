...
<div class="space-y-6" x-data="galleryManager()">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @foreach($galleries as $gallery)
                <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col group relative">
                    ...
                    <div class="relative aspect-4/3 bg-slate-100 overflow-hidden cursor-pointer" @click="openLightbox('{{ $gallery->url }}', '{{ $gallery->title }}', '{{ $gallery->category }}')">
                        <img src="{{ $gallery->url }}" 
                             alt="{{ $gallery->title }}" 
                             loading="lazy"
                             onerror="this.onerror=null; this.src='{{ asset('images/placeholder-gallery.svg') }}';"
                        ...
                        <div class="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-slate-900/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-2 p-4">
                            <!-- Zoom Lightbox -->
                            <button type="button" 
                                     @click.stop="openLightbox('{{ $gallery->url }}', '{{ $gallery->title }}', '{{ $gallery->category }}')"
                            ...
                            <!-- Copy URL -->
                            <button type="button" 
                                     @click.stop="copyToClipboard('{{ $gallery->url }}')"
    ...
    <div x-show="lightboxOpen" 
        ...
        <div class="max-w-5xl max-h-[80vh] w-full h-full flex items-center justify-center my-auto p-4">
            <img :src="lightboxSrc" onerror="this.onerror=null; this.src='{{ asset('images/placeholder-gallery.svg') }}';" class="max-w-full max-h-[75vh] object-contain rounded-2xl shadow-2xl border border-white/10">
        </div>
