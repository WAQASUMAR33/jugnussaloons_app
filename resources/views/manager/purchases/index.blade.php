...
<div x-data="{ 
    ...
    items: [
        { product_id: '', search_term: '', open: false, quantity: 1, unit_price: 0, subtotal: 0 }
    ],
    paidAmount: 0,
    addItem() {
        this.items.push({ product_id: '', search_term: '', open: false, quantity: 1, unit_price: 0, subtotal: 0 });
    },
    removeItem(index) {
            ...
            <form method="POST" action="{{ route('manager.purchases.store') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="account_id" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 text-sm font-medium focus:ring-2 focus:ring-amber-600">
                            <option value="">Select Supplier Account</option>
                            @forelse($suppliers as $supplier)
                                <option value="{{ $supplier->id }}">
                                    {{ $supplier->name }} (Category: {{ $supplier->category->title ?? 'General' }} | Balance: {{ number_format($supplier->balance, 2) }})
                                </option>
                            @empty
                                <option value="" disabled class="text-rose-600 font-bold">⚠️ No Supplier accounts found. Please add a Supplier account under Customer Accounts.</option>
                            @endforelse
                        </select>
                    </div>
