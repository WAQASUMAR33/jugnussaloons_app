            ...
            <nav class="flex-1 px-3 py-4 space-y-2.5 overflow-y-auto" 
                 x-data="{ 
                     ...
                     openMenus: {
                         ...
                         accounts: {{ request()->routeIs('manager.accounts.*') || request()->routeIs('manager.account-categories.*') || request()->routeIs('manager.ledger.*') || request()->routeIs('manager.expenses.*') || request()->routeIs('manager.expense-categories.*') ? 'true' : 'false' }},
                ...
                <div x-show="!menuQuery || $el.innerText.toLowerCase().includes(menuQuery.toLowerCase())" class="space-y-1">
                    ...
                    <div x-show="openMenus.accounts || menuQuery.length > 0" class="pl-3.5 space-y-1 border-l-2 border-amber-200/80 ml-3.5 mt-1.5">
                        ...
                        <a href="{{ route('manager.expenses.index') }}" 
                           x-show="!menuQuery || $el.innerText.toLowerCase().includes(menuQuery.toLowerCase())"
                           class="flex items-center gap-2.5 px-3 py-2 text-xs font-bold transition-all group {{ request()->routeIs('manager.expenses.*') ? 'bg-indigo-600 text-white shadow-md shadow-indigo-100 font-extrabold' : 'text-slate-600 hover:bg-indigo-50/70 hover:text-indigo-600 hover:translate-x-0.5' }}">
                            <span class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('manager.expenses.*') ? 'bg-white' : 'bg-slate-300 group-hover:bg-indigo-500' }}"></span>
                            <span>Expense Management</span>
                        </a>

                        <a href="{{ route('manager.expense-categories.index') }}" 
                           x-show="!menuQuery || $el.innerText.toLowerCase().includes(menuQuery.toLowerCase())"
                           class="flex items-center gap-2.5 px-3 py-2 text-xs font-bold transition-all group {{ request()->routeIs('manager.expense-categories.*') ? 'bg-indigo-600 text-white shadow-md shadow-indigo-100 font-extrabold' : 'text-slate-600 hover:bg-indigo-50/70 hover:text-indigo-600 hover:translate-x-0.5' }}">
                            <span class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('manager.expense-categories.*') ? 'bg-white' : 'bg-slate-300 group-hover:bg-indigo-500' }}"></span>
                            <span>Expense Categories</span>
                        </a>

                        <a href="{{ route('manager.payroll.index') }}" 
                           x-show="!menuQuery || $el.innerText.toLowerCase().includes(menuQuery.toLowerCase())"
                           class="flex items-center gap-2.5 px-3 py-2 text-xs font-bold transition-all group {{ request()->routeIs('manager.payroll.*') ? 'bg-indigo-600 text-white shadow-md shadow-indigo-100 font-extrabold' : 'text-slate-600 hover:bg-indigo-50/70 hover:text-indigo-600 hover:translate-x-0.5' }}">
                            <span class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('manager.payroll.*') ? 'bg-white' : 'bg-slate-300 group-hover:bg-indigo-500' }}"></span>
